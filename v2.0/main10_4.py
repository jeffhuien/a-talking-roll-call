...
# from python_sdk.tcloud_tts import txspeak  #腾讯语音api，具体使用教程见腾讯官方文档
...
def num(x):
    ...
    while n < x:
        ...
        nameLs.remove(name)
        out_of_list.append(name)

        if len(nameLs) < 3:
            nameLs.clear()
            nameLs = backName[:]
        s = ""
        for i in out_of_list:
            s += i + "    "
        # if x==1:
        #     names1.append(name)
        #     return names1
...
def insert_item():
    dir1 = os.getcwd()
    list1 = []
    for root, child, files in os.walk(dir1):
        for file in files:
            # print(根目录, '--->', file)
...
def one1():
    ...
    print(namelist.get())
    log()
    lastname.insert(END, out_of_list[-1])
    bgcolor()
...
timelabel = Label(root, textvariable=timetext, font=("楷体", 15), bg="lightblue").place(rely=0.2, relx=0)
l2 = Label(root, text="同学你别慌 运气来了挡也挡不住", font=("华文彩云", 18, "italic"), fg="#168AC4", bg=root.cget("bg"))
...
b4 = Button(root, text="点名4", command=four, width=8, height=1, relief=FLAT, bg="#f46ec6")
...
l4 = Label(root, text="历史数据请移步至同文件夹的<log.log>", font=("ZHSRuiXian-yolan", 20), bg="#000000", fg="#f46ec6", height=1, width=58, anchor=CENTER)
