...
LastEditors: GAO--HUI
LastEditTime: 2022-01-12 16:01:07
FilePath: \点名器_by_计181髙輝‘’\main_app.py
'''
...
class App:
    ...
    def __init__(self) -> None:
        ...
        self.l0.place(relx=0.71-0.1, rely=0.2)

        self.online = ttk.Combobox(self.root, width=5)
        self.online["value"] = ["联网", "不联网"]
        self.online.current(1)
        self.online.place(relx=0.80-0.1, rely=0.2)

        self.voice = ttk.Combobox(self.root, width=5)
        self.voice["value"] = ["读出结果", "不读"]
        self.voice.current(1)
        self.voice.place(relx=0.80, rely=0.2)

        self.txtlist = ttk.Combobox(
            self.root, width=12  # , textvariable=self.combobox_text
        )
        self.txtlist.bind("<<ComboboxSelected>>", self.show)

        self.txtlist.place(relx=0.87, rely=0.2)
    ...
    def source_GUI(self):
        ...
        self.l0.pack_forget()
        self.l0.place(relx=0.71, rely=0.2)

        self.online.pack_forget()
        self.online.place(relx=0.80, rely=0.2)
    ...
    def one1(self):
        ...
        self.txtlist.pack(side=LEFT, anchor=NW, padx=10)
